...
if (( RC == 0 )); then
    # 4a. SUCCESS --- state transition only (NO auto-commit)
    # 커밋은 사용자가 직접 수행. 하네스는 상태 전이만 담당.
    ...
    printf 'HARNESS_STATE_TRANSITION task=%s from=in_progress to=done hooks_passed=%s\n' "$TASK_ID" "$HOOKS_PASSED"
    harness_log "DONE $TASK_ID (hooks=$HOOKS_PASSED)"
    exit 0
fi
