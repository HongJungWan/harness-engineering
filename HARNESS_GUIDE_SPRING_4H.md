# 하네스 엔지니어링 가이드 — 4시간 / Spring / MySQL 프로파일
...
|---|---|
| 시간 | **4시간 (240분)** |
| 스택 | Java 21, Spring Boot 3.x (Jakarta EE), MySQL 8.x (또는 과제 지정 RDBMS), Docker Compose |
| 실행 전제 | **단일 인스턴스**, **AWS 관리형 서비스 비의존** |
| 모듈 구조 | **Gradle 멀티 모듈** (multi-project build) |
| 외부 API | **있을 수 있음** (알림톡·결제·지도 등 — ACL 규칙 D-5 포함) |
...
- M-1/M-4/D-5 의 grep 샘플 BC·어댑터 이름 (lease|tenant|kakao|notification 등) 은 Phase 1
  분석으로 파악한 실제 과제 BC·외부 클라이언트 이름으로 반드시 치환하라. 치환 누락은
  무관한 단어 오탐(false positive) 또는 실제 위반 누락(false negative) 을 유발한다.
...
아키텍처 프로파일 — Single-Instance Spring + MySQL / RDBMS (기본 전부 포함, 문제 맥락과 안 맞는 것만 제외):
...
[D — DDD 준수]
  ...
  D-4  상태 전이 캡슐화 — 엔티티에 public setter 금지 (상태 변경은 명명된 도메인 메서드로만)
        ...
        → 매치되면 FAIL
  D-5  Anti-Corruption Layer — 외부 API 클라이언트/DTO 가 domain 에 침투 금지
       (외부 통신은 acl/ 또는 adapter/ 경계에서 도메인 모델로 변환)
        grep -rnE 'import\s+[^;]*\.(external|client|api\.dto|gateway|kakao|alimtalk|notification\.client|payment\.gateway)\.' modules/<mod>/src/main/java/*/domain/
        → 매치되면 FAIL. 실제 어댑터 패키지명은 Phase 1 에서 파악한 이름으로 치환하라.

[M — 멀티 모듈 준수]
  M-1  모듈 간 직접 import 금지 — 다른 BC 의 패키지 import 불가
       (BC 쌍별로 1개 함수씩 생성하는 것을 권장)
        grep -rnE 'import\s+[^;]*\.(lease|tenant|landlord|bill|payment|notification|vacancy|property)\.' modules/<mod>/src/main/java/   # 샘플. 실제 BC 이름으로 치환 필수. 자기 BC 는 제외.
        → 매치되면 FAIL
  ...
  M-4  Cross-BC 직접 호출 금지 — application 계층에서 타 모듈 service/facade 직접 호출 금지
       (내부 @EventListener 또는 outbox 로만 연동)
        grep -rnE 'import\s+[^;]*\.(lease|tenant|landlord|bill|payment|notification|vacancy|property)\.application\.' modules/<mod>/src/main/java/*/application/   # 샘플. 실제 BC 이름으로 치환 필수.
        → 매치되면 FAIL (M-1 보강)
  M-5  동시성 제어 명시 — 금전/재고/상태 변경 usecase 에 `@Version`(낙관) 또는 `FOR UPDATE`(비관)
       또는 MySQL `GET_LOCK` / PG `pg_advisory_*_lock` 중 하나가 존재
        grep -rnE '(@Version|FOR\s+UPDATE|GET_LOCK|pg_advisory)' modules/<mod>/src/main/java/
...
# [B — 배치 / 스케줄러 준수 (비활성. 과제에 정기 처리 / Spring Batch / @Scheduled 사용 시 활성화)]
#   B-1  배치 멱등성 — @Scheduled / Tasklet / ItemProcessor 사용 시 processed_events 또는
#        @IdempotentConsumer 참조 필수 (재실행 안전성)
#         grep -rnE '(@Scheduled|implements\s+Tasklet|extends\s+ItemProcessor)' modules/*/src/main/java/
#         → 매치된 클래스가 processed_events / @IdempotentConsumer 미참조 시 FAIL
#   B-2  장시간 트랜잭션 금지 — @Transactional(timeout) 100초 이상 금지 (chunk 단위 분할 강제)
#         grep -rnE '@Transactional[^)]*timeout\s*=\s*\d{3,}' modules/*/src/main/java/
#         → 매치되면 FAIL
# 활성화 방법: 위 # 주석을 제거한 뒤 check_B1 / check_B2 함수를 check.sh 에 추가하고
# FAST_RULES 배열에 등록. 과제가 배치/스케줄러를 언급하지 않으면 그대로 비활성 유지.

판정 방향 요약:
  - "존재해야 함" (매치 0이면 FAIL): D-3, M-3, M-5, E-2, E-3, E-4, E-5, I-4
  - "없어야 함"   (매치 있으면 FAIL): D-1, D-2, D-4, D-5, M-1, M-2, M-4, E-1
...
| 브로커 | SQS FIFO, Kafka, @FeignClient | DB outbox + @Scheduled poller + ApplicationEventPublisher |
| DLQ | SQS dead-letter 토픽 | `dead_letter_events` DB 테이블 |
| 동시성 | RedissonClient / `@DistributedLock` 필수 | `@Version` / `FOR UPDATE` / MySQL `GET_LOCK` / PG advisory lock 중 택 1 |
| D-5 ACL | 필수 | 필수 (알림·결제 등 외부 호출 가정) |
| 체크포인트 | 없음 | 60분 / 120분 2회 (드리프트 방지) |
| Phase 4 | check.sh + simplify + security-review | + 실제 `docker compose up` + README walk |
| I 블록 | 없음 | 신규 3개 (docker config / Dockerfile 멀티스테이지 / README) |
| B 블록 | 없음 | 주석 상태 (배치 과제면 활성화) |

## 드롭·치환한 원본 규칙과 사유

- **M-5 (RedissonClient 분산 락)** → **치환**: 단일 인스턴스라 분산 락은 불필요. 다만 금전/재고 동시성은 여전히 중요하므로 **DB 레벨 락 (`@Version` / `FOR UPDATE` / MySQL `GET_LOCK` / PG `pg_advisory_*_lock`) 명시 사용** 을 새 필수 패턴으로 요구.
- **E-2 (SQS publish Relay)** → **치환**: 동일 구조이되 publisher 가 `ApplicationEventPublisher` 또는 로컬 핸들러. Relay 는 `@Scheduled` poller 로 동일하게 분리.
- **E-4 (SQS/Kafka DLQ 토픽)** → **치환**: 같은 목적(실패 격리) 을 DB `dead_letter_events` 테이블로 달성.
- **D-5 (ACL)** → **유지**: 자리톡류 과제는 알림톡·결제·지도 등 외부 API 연동 비중이 크므로 도메인 오염 차단을 위해 그대로 둔다. 외부 API 가 없다고 Phase 1 에서 확인되면 "제외된 규칙" 섹션에 근거와 함께 기록하고 드롭.
- **AWS SDK import 금지 규칙 (원래 I-1 후보)** → **미도입**: hook 으로 강제할 가치 대비 false-positive 리스크가 크다는 판단. "AWS 관리형 서비스 비의존" 은 설계 원칙 (CLAUDE.md [설계 가이드라인]) 으로만 유지하고 hook 으로 잡지 않는다.
