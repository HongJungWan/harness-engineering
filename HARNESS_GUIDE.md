...
세팅을 마친 뒤 Claude Code 에 **아래 프롬프트 하나만** 붙여넣는다. `<여기에 문제 전문을 붙여넣으세요>` 부분만 실제 문제로 교체하면 Claude 가 Phase 1~4 를 한 턴 안에 자율적으로 실행한다 (각 Phase 사이 사용자 확인 없음).
